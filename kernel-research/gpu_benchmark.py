...
This is the standalone top-level entry point for Lambda deployment.
It imports kernel implementations from kernel-research/harness/ and runs
all GPU benchmarks, writing results to results/results.jsonl.

Usage:
    # On Lambda (from kernel-research/ directory):
    python gpu_benchmark.py

    # Run single kernel:
    python gpu_benchmark.py --kernel triton_flash

    # Run single batch size:
...
Prerequisites:
    ...
    - Triton (for triton_flash_attention kernel)
    - Files in harness/ directory: gpu_kernels.py, baseline_kernels.py,
      optimized_kernels.py, harness.py

Output schema per record (appended to results/results.jsonl):
{
  "hypothesis_id":          "gpu-h001-triton-flash",
...
# ---------------------------------------------------------------------------
# Baseline / input-generation imports
# ---------------------------------------------------------------------------
...
SPEEDUP_THRESHOLD  = 1.10    # 10% wall-clock improvement over baseline
VARIANCE_THRESHOLD = 1.05    # p99 / p50 ratio
BATCH_SIZES = [1, 8, 32]
...
def _evaluate(baseline_p50, kernel_p50, max_err, p50, p99, max_err_threshold):
    ...
    if vr < VARIANCE_THRESHOLD:
        parts.append(f"variance p99/p50={vr:.3f} OK")
    else:
        parts.append(f"variance p99/p50={vr:.3f} WARNING")
...
def _make_record(hypothesis_id, kernel_name, kernel_type, batch_size, baseline_name,
                  stats, max_err, peak_memory_bytes, device, max_err_threshold=1e-4):
...
def bench_triton_flash_attention(batch_size, device, use_gpu):
    ...
    max_err = correctness_check(_sdpa(q, k, v), triton_flash_attention(q, k, v))
...
def bench_compiled_attention(batch_size, device, use_gpu):
    ...
    record = _make_record("gpu-h002-compiled-attn", "compiled_attention", "attention",
                           batch_size, "naive_attention", stats, max_err, peak_mem, device, 1e-4)
    time_fn = _time_fn_gpu if use_gpu else _time_fn_cpu
...
def bench_fp16_gemm(batch_size, device, use_gpu):
    ...
    peak_mem = _peak_cuda_memory_bytes()
    max_err = correctness_check(_fp32_gemm(a_f32, b_f32), fp16_gemm(a_f16, b_f16))
...
def run_all_benchmarks(kernel_filter=None, batch_sizes=None, write_results=True):
    ...
    if not use_gpu:
        print("WARNING: CUDA is not available. GPU benchmarks will fail or produce "
              "meaningless results. Deploy to the H100 Lambda instance.")
...
def print_summary(records):
    ...
    for rec in records:
        ...
        print(fmt.format(
            rec["kernel_name"][:30],
            rec["batch_size"],
            f"{rec['baseline_p50_us']:.1f}",
            f"{rec['kernel_p50_us']:.1f}",
            f"{rec['speedup']:.3f}x",
...
def main():
    ...
    parser.add_argument("--kernel", choices=list(ALL_BENCHMARKS.keys()), default=None,
                        help="Run only this kernel benchmark (default: all)")
    parser.add_argument("--batch_size", type=int, default=None,
                        help="Single batch size to test (default: 1,8,32)")
    parser.add_argument("--no-write", action="store_true",
                        help="Do not write results to results.jsonl")
